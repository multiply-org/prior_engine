...
def test_prior_engine():
    ...
    assert './aux_data/Static/Vegetation/Priors//Priors_cab_060_global.vrt' == priors['cab']['database']

    assert 'lai' in priors.keys()
    assert 1 == len(priors['lai'])
    assert 'database' in priors['lai']
    assert './aux_data/Static/Vegetation/Priors//Priors_lai_060_global.vrt' == priors['lai']['database']

    assert 'sm' in priors.keys()
    assert 1 == len(priors['sm'])
    assert 'climatology' in priors['sm']
    assert './aux_data/Climatology/SoilMoisture//ESA_CCI_SM_CLIM_3.geotiff' == priors['sm']['climatology']
